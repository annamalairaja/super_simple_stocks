...
class Trade:
    """collection of shares at a definite price per share"""
...
class Stock(abc.ABC):
    ...
    def record_trade(self, trade: Trade):
        ...
        if not isinstance(trade, Trade):
            msg = f"Argument trade={trade} should be of type Trade."
            raise TypeError(msg)
        elif self.ticker_symbol is not trade.ticker_symbol:
            msg = f"Argument trade={trade} does not belong to this stock."
            raise ValueError(msg)
        else:
...
class GlobalBeverageCorporationExchange:
    ...
    def __init__(self,
        ...
        if len(stocks) > 0:
            ...
            msg = f"Argument stocks={stocks} should be a non empty sequence."
            raise ValueError(msg)
